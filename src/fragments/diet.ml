...
let rec node x y l r =
  ...
  if hl > hr + 2 then
    let[@warning "-partial-match"] (Node { x= lx; y= ly; l= ll; r= lr; _ }) =
      l
    in
    if height ll >= height lr then node lx ly ll (node x y lr r)
    else
      let[@warning "-partial-match"] (Node { x= lrx; y= lry; l= lrl; r= lrr; _ })
          =
        lr
      in
      node lrx lry (node lx ly ll lrl) (node x y lrr r)
  else if hr > hl + 2 then
    let[@warning "-partial-match"] (Node { x= rx; y= ry; l= rl; r= rr; _ }) =
      r
    in
    if height rr >= height rl then node rx ry (node x y l rl) rr
    else
      let[@warning "-partial-match"] (Node { x= rlx; y= rly; l= rll; r= rlr; _ })
          =
        rl
      in
