let src = Logs.Src.create "tls-mkernel"
...
let handle flow tls str =
  ...
  | Ok (state, eof, `Response resp, `Data data) ->
      ...
      let state =
        Option.(
          value ~default:state (map (fun `Eof -> half_close state `read) eof))
      in
      flow.state <- state;
...
let rec read_in flow ?(off = 0) ?len buf =
  ...
  let write_in res =
    ...
    let linger =
      if mlen < rlen then Some (String.sub res mlen (rlen - mlen)) else None
    in
    flow.linger <- linger;
    mlen
  in
...
let rec drain_handshake flow =
  ...
  | _ ->
      ...
      push_linger flow mcs; drain_handshake flow
...
let client_of_fd conf ?host ?ip fd =
  let conf' =
...
let write flow ?(off = 0) ?len str =
  ...
  if off < 0 || len < 0 || off > String.length str - len then
    invalid_arg "Tls_mkernel.write";
  if len > 0 then writev flow [ String.sub str off len ]

let read t ?(off = 0) ?len buf =
  let len = Option.value ~default:(Bytes.length buf - off) len in
  if off < 0 || len < 0 || off > Bytes.length buf - len then
    invalid_arg "Tls_mkernel.read";
  if t.rd_closed then 0 else try read_in t ~off ~len buf with End_of_file -> 0

let rec really_read_go t off len buf =
  let len' = read t buf ~off ~len in
  if len' == 0 then raise End_of_file
  else if len - len' > 0 then really_read_go t (off + len') (len - len') buf

let really_read t ?(off = 0) ?len buf =
  let len = Option.value ~default:(Bytes.length buf - off) len in
  if off < 0 || len < 0 || off > Bytes.length buf - len then
    invalid_arg "Tls_mkernel.really_read";
